...
\subsection{Policy Rationale for the Requirements}
\label{sec:wqg-policy}

Before we state the six requirements formally, we justify each one
on policy grounds. The weights $(3,2,1)$ are derived as a
\emph{consequence} of these policy choices; if the choices change
(different committee size, different liability model, different
regulatory regime), the resulting weights change too. What follows
is the claim that the policy choices themselves are defensible,
not arbitrary.

\paragraph{R1 (two stewards alone pass a standard quorum): why.}
This is the \emph{simple-majority-of-peers} principle from
consortium-governance practice. In a committee of $n$ equal-standing
principals, the routine-operations threshold is $\lceil n/2 \rceil$
out of $n$. For the reference committee of 3 stewards that is 2.
The alternative -- requiring all stewards for every routine
operation -- collapses into a single-point veto problem in which
the absence or disagreement of any one steward paralyses the
system, which historically produces shadow out-of-band processes
(``we'll just do it by email''). V-Lake reserves steward unanimity
for the \emph{critical} class (\texttt{all\_stew=True},
Table~\ref{tab:quorum}), which is where it belongs. R1 therefore
governs the floor for routine governance, not the ceiling.

Note that R1 is a claim about the reference committee of 3
stewards. For a larger committee (5, 7, $\ldots$ stewards) the
same simple-majority-of-peers principle yields a higher threshold,
and the weight triple is rederived accordingly. We treat the
reference committee as the minimal interesting case; scaling is
discussed in \S\ref{sec:stages}.

\paragraph{R2 (one steward alone cannot pass): why.}
This is the defining property of a multi-principal governance
scheme. If any single steward can ratify a routine operation
alone, they are a dictator, and every other steward's membership
is ceremonial. The reason V-Lake has multiple stewards at all is
that no single principal should unilaterally affect shared data;
R2 is that commitment translated into the math. Without R2 the
weighted-voting layer is theatre.

\paragraph{R3 (a custodian is pivotal in some winning coalition):
why.}
A role whose vote never tips any winning coalition has no actual
influence. If the math is such that custodian votes never matter,
the operational staff who run the system have been given a
ceremonial badge but no actual power, and they become rubber
stamps. R3 is the minimal guarantee that custodians' votes are
load-bearing somewhere -- specifically in the marginal coalitions
where a steward plus a custodian + analyst just crosses quorum.
Without R3, the governance layer would claim custodian
participation but deliver none.

\paragraph{R4 (non-stewards alone cannot pass): why.}
Classic separation of powers. The people who \emph{operate} the
system (custodians) and \emph{consume} data from it (analysts)
should not be the people who \emph{change the rules} of the system.
Regulators view combined operate-and-rule-change authority as a
control-environment red flag (e.g.\ COSO, SOC\,2 CC1.3). R4 says:
to change governance state you need at least one executive-level
principal (steward) on board.

\paragraph{R5 (strict ordering $0 < a < c < s$): why.}
Weights should track \emph{liability}, not head-count. Stewards
face the maximum regulatory exposure (e.g.\ GDPR administrative
fines up to 4\% of global revenue, SOX / MiFID\,II institutional
penalties). Custodians face operational and contractual liability
(DPA clauses, AUPs, audit findings). Analysts are access-scope-
bounded and bear the least liability, though they too carry
individual accountability for their queries. Weights that reverse
this ordering would mean the roles with the least skin in the
game dominate the vote -- which every governance framework we
know of warns against.

\paragraph{R6 (integer weights): why.}
Two reasons, one practical and one readability. Practical: Solidity
does not have native rationals, and implementing a fixed-point
weight system inflates gas and invites rounding bugs. Readability:
``weight 3'' is a discrete number a steward can recognise in an
audit trail; ``weight 3.14'' or ``weight 1/$\sqrt{2}$'' immediately
raises the question of where the fraction came from. Integers keep
the on-chain state self-documenting.

\paragraph{What the reader should take from this.}
None of R1--R6 is a mathematical theorem. They are governance-
policy choices we defend by appeal to consortium practice,
separation of duties, and liability tracking. Given those choices,
Proposition~\ref{prop:unique} then proves that $(3,2,1)$ is the
unique minimal integer solution. A reviewer who disagrees with a
policy choice can restate R1--R6 and rederive the triple; the
contribution of the paper is the framework in which that rederivation
is principled and explicit, not hand-waved.


\subsection{Weight Derivation}

V-Lake assigns integer weights: Steward~$s{=}3$, Custodian~$c{=}2$,
Analyst~$a{=}1$. We state six requirements (whose policy
rationales are given in \S\ref{sec:wqg-policy}) and prove that
...
verification, and the Merkle construction are unchanged.
...
derivation in Proposition~\ref{prop:unique}. They change what the
